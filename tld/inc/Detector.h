...
#include "qtld_global.h"

class Lk;
class Fern;

typedef std::pair<cv::Mat_<unsigned>* /*img*/, cv::Mat_<unsigned>* /*blur*/ >ImagePairType;
...
public:
	QTLD_EXPORT Detector();
	QTLD_EXPORT ~Detector();
	void run();
	QTLD_EXPORT bool init(IplImage& image, const QRectF& region);
	void display(int runtime, int frameNumber = 0);
	QTLD_EXPORT void processFrame(IplImage* frame);
	bool initialised(void) { return m_initialised; };

signals:
	void newFrame(IplImage* image);
	void newPexPixmap(QPixmap pixmap);
	void pexCount(int count);
	void newNexPixmap(QPixmap pixmap);
	void nexCount(int count);
	void status(const QString message);
	void boundingRect(QRectF rect);
	void info(QString infoString);
...
protected:
	...
	void imagePatch(const cv::Mat_<unsigned>& inMat, const cv::Mat_<double>& bb, cv::Mat_<unsigned char>& outMat, const ParOptionsType& init, double randomise = -1);
	void warp(const cv::Mat_<unsigned>& im, const cv::Mat_<double>& H, const cv::Mat_<double>& B, cv::Mat_<double>& outMat);
	...
	void blur(IplImage& image, cv::Mat_<unsigned>& outMat, double sigma = 1.5);
	void emitImage(cv::Mat& mat);
	QPixmap mat2pixmap(cv::Mat& inMat);
...
private:
	...
	IplImage* m_image;
	//blur // bb gaussian filtered
	//grid // 6 x 20967 double
	//scales // 2 x 12 double
	...
	cv::Mat_<unsigned int> m_imgSize;
